...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("dng", "isPolymorphed") || false;
  }
  ...
  prepareDerivedData() {
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag('dng', 'transformOptions');
      const original = game.actors?.get(this.getFlag('dng', 'originalActor'));
      if (original) {
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.DNG.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.DNG.CR_EXP_LEVELS[cr];
  }
  ...
  static async getClassFeatures({className="", subclassName="", level=1, priorLevel=0}={}) {
    ...
    const clsConfig = CONFIG.DNG.classFeatures[className];
    if (!clsConfig) return [];
  ...
  _prepareSkills(actorData, bonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = DNG.characterFlags;
    const athlete = flags.remarkableAthlete;
    const joat = flags.jackOfAllTrades;
  ...
  _computeSpellcastingDC(actorData) {
    ...
    for ( let i of this.items ) {
      ...
      if ( save?.ability ) {
        ...
        const ability = CONFIG.DNG.abilities[save.ability];
        i.labels.save = game.i18n.format("DNG.SaveDC", {dc: save.dc || "", ability});
      }
    }
  }
  ...
  _computeSpellcastingProgression (actorData) {
    ...
    const slots = DNG.SPELL_SLOT_TABLE[levels - 1] || [];
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("dng", "currencyWeight") ) {
      const currency = actorData.data.currency;
      const numCoins = Object.values(currency).reduce((val, denom) => val += Math.max(denom, 0), 0);
      weight += numCoins / CONFIG.DNG.encumbrance.currencyPerWeight;
    }
    ...
    if ( this.getFlag("dng", "powerfulBuild") ) mod = Math.min(mod * 2, 8);

    // Compute Encumbrance percentage
    weight = weight.toNearest(0.1);
    const max = actorData.data.abilities.str.value * CONFIG.DNG.encumbrance.strMultiplier * mod;
  ...
  async update(data, options={}) {
    ...
    if ( newSize && (newSize !== getProperty(this.data, "data.traits.size")) ) {
      let size = CONFIG.DNG.tokenSizes[newSize];
  ...
  async useSpell(item, {configureDialog=true}={}) {
    ...
    const usesSlots = (lvl > 0) && CONFIG.DNG.spellUpcastModes.includes(itemData.preparation.mode);
    ...
    if ( usesSlots && consumeSlot && (lvl > 0) ) {
      ...
      if ( slots === 0 || Number.isNaN(slots) ) {
        return ui.notifications.error(game.i18n.localize("DNG.SpellCastNoSlots"));
      }
      await this.update({
    ...
    if ( limitedUses && consumeUse ) {
      ...
      if ( uses <= 0 ) ui.notifications.warn(game.i18n.format("DNG.ItemNoUses", {name: item.name}));
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("dng", "reliableTalent"));

    // Roll and return
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("DNG.SkillPromptTitle", {skill: CONFIG.DNG.skills[skillId]}),
      halflingLucky: this.getFlag("dng", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: {"flags.dng.roll": {type: "skill", skillId }}
    });
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.DNG.abilities[abilityId];
    new Dialog({
      title: game.i18n.format("DNG.AbilityPromptTitle", {ability: label}),
      content: `<p>${game.i18n.format("DNG.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("DNG.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("DNG.ActionSave"),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    const feats = this.data.flags.dng || {};
    if ( feats.remarkableAthlete && DNG.characterFlags.remarkableAthlete.abilities.includes(abilityId) ) {
    ...
    const rollData = mergeObject(options, {
      ...
      messageData: {"flags.dng.roll": {type: "ability", abilityId }}
    });
    rollData.speaker = options.speaker || ChatMessage.getSpeaker({actor: this});
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("DNG.SavePromptTitle", {ability: label}),
      halflingLucky: this.getFlag("dng", "halflingLucky"),
      messageData: {"flags.dng.roll": {type: "save", abilityId }}
    });
    rollData.speaker = options.speaker || ChatMessage.getSpeaker({actor: this});
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.data.data.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3)) {
      ui.notifications.warn(game.i18n.localize("DNG.DeathSaveUnnecessary"));
      return null;
    }
    ...
    if ( this.getFlag("dng", "diamondSoul") ) {
      parts.push("@prof");
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.localize("DNG.DeathSavingThrow"),
      speaker: speaker,
      halflingLucky: this.getFlag("dng", "halflingLucky"),
      targetValue: 10,
      messageData: {"flags.dng.roll": {type: "death"}}
    });
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        await ChatMessage.create({content: game.i18n.format("DNG.DeathSaveCriticalSuccess", {name: this.name}), speaker});
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        await ChatMessage.create({content: game.i18n.format("DNG.DeathSaveSuccess", {name: this.name}), speaker});
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        await ChatMessage.create({content: game.i18n.format("DNG.DeathSaveFailure", {name: this.name}), speaker});
      }
    }
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    if ( !cls ) {
      ui.notifications.error(game.i18n.format("DNG.HitDiceWarn", {name: this.name, formula: denomination}));
      return null;
    }

    // Prepare roll data
    const parts = [`1${denomination}`, "@abilities.con.mod"];
    const title = game.i18n.localize("DNG.HitDiceRoll");
    ...
    const roll = await damageRoll({
      ...
      messageData: {"flags.dng.roll": {type: "hitDie"}}
    });
    if ( !roll ) return null;
  ...
  async shortRest({dialog=true, chat=true, autoHD=false, autoHDThreshold=3}={}) {
    ...
    if ( chat ) {
      ...
      let restFlavor;
      switch (game.settings.get("dng", "restVariant")) {
        case 'normal': restFlavor = game.i18n.localize("DNG.ShortRestNormal"); break;
        case 'gritty': restFlavor = game.i18n.localize(newDay ? "DNG.ShortRestOvernight" : "DNG.ShortRestGritty"); break;
        case 'epic':  restFlavor = game.i18n.localize("DNG.ShortRestEpic"); break;
      }

      // Summarize the health effects
      let srMessage = "DNG.ShortRestResultShort";
      if ((dhd !== 0) && (dhp !== 0)) srMessage = "DNG.ShortRestResult";
  ...
  async longRest({dialog=true, chat=true, newDay=true}={}) {
    ...
    let restFlavor;
    switch (game.settings.get("dng", "restVariant")) {
      case 'normal': restFlavor = game.i18n.localize(newDay ? "DNG.LongRestOvernight" : "DNG.LongRestNormal"); break;
      case 'gritty': restFlavor = game.i18n.localize("DNG.LongRestGritty"); break;
      case 'epic':  restFlavor = game.i18n.localize("DNG.LongRestEpic"); break;
    }

    // Determine the chat message to display
    if ( chat ) {
      let lrMessage = "DNG.LongRestResultShort";
      if((dhp !== 0) && (dhd !== 0)) lrMessage = "DNG.LongRestResult";
      else if ((dhp !== 0) && (dhd === 0)) lrMessage = "DNG.LongRestResultHitPoints";
      else if ((dhp === 0) && (dhd !== 0)) lrMessage = "DNG.LongRestResultHitDice";
      ChatMessage.create({
        user: game.user._id,
        speaker: {actor: this, alias: this.name},
        flavor: restFlavor,
  ...
  convertCurrency() {
    ...
    const convert = CONFIG.DNG.currencyConversion;
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("dng", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("DNG.PolymorphWarn"));
    }

    // Get the original Actor data and the new source data
    const o = duplicate(this.data);
    o.flags.dng = o.flags.dng || {};
    o.flags.dng.transformOptions = {mergeSkills, mergeSaves};
    ...
    for ( let k of Object.keys(abilities) ) {
      ...
      if ( keepPhysical && ["str", "dex"].includes(k) ) abilities[k] = oa;
      else if ( keepMental && ["int", "wis", "cha", "mag"].includes(k) ) abilities[k] = oa;
    ...
    if (!keepClass && d.data.details.cr) {
      d.items.push({
        type: 'class',
        name: game.i18n.localize('DNG.PolymorphTmpClass'),
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.dng.originalActor ) d.flags.dng.originalActor = this.id;
    d.flags.dng.isPolymorphed = true;
    ...
    Hooks.callAll('dng.transformActor', this, target, d, {
  ...
  async revertOriginalForm() {
    ...
    if ( !this.owner ) {
      return ui.notifications.warn(game.i18n.localize("DNG.PolymorphRevertWarn"));
    }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: 'DNG.PolymorphRestoreTransformation',
