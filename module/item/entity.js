...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.DNG.areaTargetTypes);
  }
  ...
  prepareData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("DNG.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("DNG.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "DNG.Attack" : "DNG.Action");
      else labels.featType = game.i18n.localize("DNG.Passive");
    }

    // Equipment Items
    else if ( itemData.type === "equipment" ) {
      labels.armor = data.armor.value ? `${data.armor.value} ${game.i18n.localize("DNG.AC")}` : "";
    }
    ...
    if ( data.hasOwnProperty("activation") ) {
      ...
      labels.recharge = `${game.i18n.localize("DNG.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
    ...
    if ( data.hasOwnProperty("actionType") ) {
      ...
      if ( save?.ability && !this.isOwned ) {
        ...
        labels.save = game.i18n.format("DNG.SaveDC", {dc: save.dc || "", ability: C.abilities[save.ability]});
      }
  ...
  async roll({configureDialog=true, rollMode=null, createMessage=true}={}) {
    ...
    const template = `systems/dng/templates/chat/${templateType}-card.html`;
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["dng.itemData"] = this.data;
    }
  ...
  async _handleResourceConsumption({isCard=false, isAttack=false}={}) {
    ...
    const typeLabel = CONFIG.DNG.abilityConsumptionTypes[consume.type];
    ...
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("DNG.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( [null, undefined].includes(consumed) ) {
      ui.notifications.warn(game.i18n.format("DNG.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    let remaining = quantity - amount;
    if ( remaining < 0) {
      ui.notifications.warn(game.i18n.format("DNG.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async _rollFeat(configureDialog) {
    ...
    if ( consume && charge.value ) {
      if ( !charge.charged ) {
        ui.notifications.warn(game.i18n.format("DNG.ItemNoUses", {name: this.name}));
        return false;
      }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      props.push(
        game.i18n.localize(data.equipped ? "DNG.Equipped" : "DNG.Unequipped"),
        game.i18n.localize(data.proficient ? "DNG.Proficient" : "DNG.NotProficient"),
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.DNG.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth.value ? game.i18n.localize("DNG.StealthDisadvantage") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.DNG.weaponTypes[data.weaponType],
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.DNG.consumableTypes[data.consumableType],
      data.uses.value + "/" + data.uses.max + " " + game.i18n.localize("DNG.Charges")
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.DNG.abilities[data.ability] || null,
      CONFIG.DNG.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("DNG.ItemTypeLoot"),
      data.weight ? data.weight + " " + game.i18n.localize("DNG.AbbreviationLbs") : null
    );
  }
  ...
  async rollAttack(options={}) {
    const itemData = this.data.data;
    const actorData = this.actor.data.data;
    const flags = this.actor.data.flags.dng || {};
    if ( !this.hasAttack ) {
      throw new Error("You may not place an Attack Roll with this Item.");
    }
    let title = `${this.name} - ${game.i18n.localize("DNG.AttackRoll")}`;
    ...
    const rollConfig = mergeObject({
      ...
      messageData: {"flags.dng.roll": {type: "attack", itemId: this.id }}
    }, options);
  ...
  rollDamage({event, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const itemData = this.data.data;
    const actorData = this.actor.data.data;
    const messageData = {"flags.dng.roll": {type: "damage", itemId: this.id }};
    ...
    const title = `${this.name} - ${game.i18n.localize("DNG.DamageRoll")}`;
    const rollConfig = {
      event: event,
      parts: parts,
      actor: this.actor,
    ...
    if ( versatile && itemData.damage.versatile ) {
      ...
      messageData["flags.dng.roll"].versatile = true;
    }
    ...
    if ( itemData.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("dng", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("DNG.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.dng.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("DNG.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "DNG.ItemRechargeSuccess" : "DNG.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("DNG.ToolCheck")}`;

    // Compose the roll data
    const rollConfig = mergeObject({
      parts: parts,
      data: rollData,
      template: "systems/dng/templates/chat/tool-roll-dialog.html",
      title: title,
      speaker: ChatMessage.getSpeaker({actor: this.actor}),
      flavor: title,
      dialogOptions: {
      ...
      halflingLucky: this.actor.getFlag("dng", "halflingLucky" ) || false,
      messageData: {"flags.dng.roll": {type: "tool", itemId: this.id }}
    }, options);
    rollConfig.event = options.event;
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("dng", "itemData");
    const item = storedData ? this.createOwned(storedData, actor) : actor.getOwnedItem(card.dataset.itemId);
    if ( !item ) {
      return ui.notifications.error(game.i18n.format("DNG.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}))
    }
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("DNG.ActionWarningNoToken"));
    return targets;
  }
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = CONFIG.DNG.spellScrollIds[level];
    ...
    const spellScrollData = mergeObject(scrollData, {
      name: `${game.i18n.localize("DNG.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
