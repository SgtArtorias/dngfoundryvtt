...
export async function d20Roll({parts=[], data={}, event={}, rollMode=null, template=null, title=null, speaker=null,
  ...
  const _roll = (parts, adv, form) => {
    ...
    if (adv === 1) {
      ...
      messageData.flavor += ` (${game.i18n.localize("DNG.Advantage")})`;
      if ( "flags.dng.roll" in messageData ) messageData["flags.dng.roll"].advantage = true;
      mods += "kh";
    }
    ...
    // Handle disadvantage
      ...
      nd = 2;
      messageData.flavor += ` (${game.i18n.localize("DNG.Disadvantage")})`;
      if ( "flags.dng.roll" in messageData ) messageData["flags.dng.roll"].disadvantage = true;
      mods += "kl";
    }
    ...
    if (ability && ability.value) {
      ...
      if (abl) {
        data.mod = abl.mod;
        messageData.flavor += ` (${CONFIG.DNG.abilities[data.ability]})`;
      }
    }
    ...
    if (reliableTalent && roll.dice[0].total < 10) {
      messageData.flavor += ` (${game.i18n.localize("DNG.FlagsReliableTalent")})`;
    }
    return roll;
  };
...
async function _d20RollDialog({template, title, parts, data, rollMode, dialogOptions, roll}={}) {
  ...
  // Render modal dialog
  template = template || "systems/dng/templates/chat/roll-dialog.html";
  let dialogData = {
    formula: parts.join(" + "),
    data: data,
    rollMode: rollMode,
...
export async function damageRoll({parts, actor, data, event={}, rollMode=null, template, title, speaker, flavor,
  ...
  const _roll = function(parts, crit, form) {
    ...
    if ( crit === true ) {
      ...
      messageData.flavor += ` (${game.i18n.localize("DNG.Critical")})`;
      if ( "flags.dng.roll" in messageData ) messageData["flags.dng.roll"].critical = true;
    }
    ...
    new Dialog({
      ...
      buttons: {
        critical: {
          ...
          label: game.i18n.localize("DNG.CriticalHit"),
          callback: html => resolve(roll(parts, true, html[0].querySelector("form")))
        },
        normal: {
          label: game.i18n.localize(allowCritical ? "DNG.Normal" : "DNG.Roll"),
